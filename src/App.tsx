...
export const loadData = async (trainSize: number = 50, testSize: number = 50) => {
  ...
  const trainData = await trainDataAll.shuffle(3)
    .take(trainSize)
    .map((res: any) => {
  ...
  const testData = await testDataAll.shuffle(3)
    .take(testSize)
    .map((res: any) => {
      return { xs: Object.values(res.xs), ys: Object.values(res.ys) }
    })
...
const knnTrain = async function () {
  try {
    ...
    nn.train(trainData.map(v=>v.xs),trainData.map(v=>v.ys))
    const Yte_predict = nn.predict(testData.map(v=>v.xs),7)
    const acc =  sum(zip(Yte_predict,testData.map(v=>v.ys)).map((v,i)=>{
      const vf = v.flat()
      if(vf[0] == vf[1]){   
        return 100
      }
